...
def main(args):
    ...
    try:
        for epoch in tqdm(range(initial_epoch, num_epochs)):
            ...
            for step, (images, captions, lengths) in enumerate(tqdm(train_loader), start=initial_step):
                ...
                if step % log_step == 0:
                    ...
                    if args.val_while_train:
                        ...
                        with torch.no_grad():
                            batch_loss_val = []
                            for val_step, (images, captions, lengths) in enumerate(tqdm(val_loader)):
                                images = utils.to_var(images, volatile=True)
                                captions = utils.to_var(
...
if __name__ == '__main__':
    ...
    parser.add_argument('--log_step', type=int,
                        default=4000, help='number of steps in between calculating loss')
    parser.add_argument('--num_hidden', type=int,
                        default=512, help='number of hidden units in the RNN')
    parser.add_argument('--embed_size', type=int,
                        default=512, help='number of embeddings in the RNN')
    parser.add_argument('--val_while_train', action='store_true')
