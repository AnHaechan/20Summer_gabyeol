    ...
    with torch.no_grad():
        try:
            ...
            for step, (images, image_ids) in enumerate(loader):
                ...
                captions = [utils.convert_back_to_text(
                    cap, vocab) for cap in captions]
                captions_formatted = [{'image_id': int(
                    img_id), 'caption': cap} for img_id, cap in zip(image_ids, captions)]
