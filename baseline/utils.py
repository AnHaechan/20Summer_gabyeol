...
def save_models(encoder, decoder, optimizer, step, epoch, losses_train, losses_val, checkpoint_path):
    ...
    checkpoint_file = os.path.join(
        checkpoint_path, 'model-%d-%d.ckpt' % (epoch + 1, step + 1))
...
def dump_losses(losses_train, losses_val, path):
    ...
    with open(path, 'wb') as f:
        try:
            pickle.dump({'losses_train': losses_train,
                         'losses_val': losses_val}, f, protocol=2)
        except:
            pickle.dump({'losses_train': losses_train,
                         'losses_val': losses_val}, f)


def convert_back_to_text(idx_arr, vocab):
    from itertools import takewhile
    blacklist = [vocab.word2idx[word] for word in [vocab.start_token()]]
    def predicate(word_id): return vocab.idx2word[word_id] != vocab.end_token()
    sampled_caption = [vocab.idx2word[word_id] for word_id in takewhile(
        predicate, idx_arr) if word_id not in blacklist]
